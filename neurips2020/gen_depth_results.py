...
parser.add_argument(
    "--load-pkl",
    action="store_true",
    help="Load predictions for a cached pickle file or recompute from scratch by feeding the data through trained models",
)
        ...
        for model_i, model_path in enumerate(model_path_list):
            ...
            for x, y, ood in datasets:
                ...
                for start_i in tqdm(np.arange(0, 3 * batch_size, batch_size)):
                    ...
                    x_batch = x[inds] / np.float32(255.0)
                    y_batch = y[inds] / np.float32(255.0)
                    ...
                    if ood:
                        ...
                        df_pred_image = pd.concat(
                            (df_pred_image, pd.DataFrame.from_dict(summary_to_add)),
                            ignore_index=True,
                        )
...
def get_prediction_summary(
    method, model_path, model, x_batch, y_batch, ood, mask_batch=None, eps=0.0
):
    ...
    summary = [
        ...
        for x, y, mu, sigma, mask in zip(
            x_batch, y_batch, mu_batch, sigma_batch, mask_batch
        )
    ]
    return summary
...
def gen_cutoff_plot(df_image, eps=0.0, ood=False, plot=True):
    ...
    df_cutoff = pd.DataFrame(columns=["Method", "Model Path", "Percentile", "Error"])

    for method, model_path_list in trained_models.items():
        for model_i, model_path in enumerate(tqdm(model_path_list)):

            df_model = df_pixel[
                (df_pixel["Method"] == method.value)
                & (df_pixel["Model Path"] == model_path)
            ]
            ...
            mean_error = [df_model[cutoff:]["Error"].mean() for cutoff in cutoff_inds]
            df_single_cutoff = pd.DataFrame(
                {
                    "Method": method.value,
                    "Model Path": model_path,
            ...
            df_cutoff = pd.concat((df_cutoff, df_single_cutoff), ignore_index=True)
    ...
    if plot:
        ...
        sns.lineplot(
            x="Percentile", y="Error", hue="Model Path", style="Method", data=df_cutoff
        )
        ...
        g = g.map_dataframe(
            sns.lineplot, x="Percentile", y="Error", hue="Model Path"
        )  # .add_legend()
        plt.savefig(
            os.path.join(output_dir, f"cutoff_eps-{eps}_ood-{ood}_trial_panel.pdf")
        )
        # plt.show()
...
def gen_calibration_plot(df_image, eps=0.0, ood=False, plot=True):
        ...
        for model_i, model_path in enumerate(tqdm(model_path_list)):
            ...
            for p in expected_p:
                ppf = scipy.stats.norm.ppf(
                    p, loc=df_model["Mu"], scale=df_model["Sigma"]
                )
            ...
            df_calibration = pd.concat((df_calibration, df_single), ignore_index=True)

    df_truth = pd.DataFrame(
        {
            "Method": Model.GroundTruth.value,
            "Model Path": "",
            "Expected Conf.": expected_p,
            "Observed Conf.": expected_p,
        }
    )
    df_calibration = pd.concat((df_calibration, df_truth), ignore_index=True)

    df_calibration["Calibration Error"] = np.abs(
        df_calibration["Expected Conf."] - df_calibration["Observed Conf."]
    )
    df_calibration["Epsilon"] = eps
    table = (
        df_calibration.groupby(["Method", "Model Path"])["Calibration Error"]
        .mean()
        .reset_index()
    )
    table = pd.pivot_table(
        table,
        values="Calibration Error",
        index="Method",
    ...
    if plot:
        ...
        sns.lineplot(
            x="Expected Conf.", y="Observed Conf.", hue="Method", data=df_calibration
        )
        ...
        g = g.map_dataframe(
            sns.lineplot, x="Expected Conf.", y="Observed Conf.", hue="Model Path"
        )  # .add_legend()
...
def gen_adv_plots(df_image, ood=False):
    ...
    df_pixel["Entropy"] = 0.5 * np.log(
        2 * np.pi * np.exp(1.0) * (df_pixel["Sigma"] ** 2)
    )

    ### Plot epsilon vs error per method
    df = (
        df_pixel.groupby([df_pixel.index, "Method", "Model Path", "Epsilon"])
        .mean()
        .reset_index()
    )
    df_by_method = (
        df_pixel.groupby(["Method", "Model Path", "Epsilon"]).mean().reset_index()
    )
    ...
    df_cumdf = pd.DataFrame(
        columns=["Method", "Model Path", "Epsilon", "Entropy", "CDF"]
    )
    ...
    for method in df["Method"].unique():
        for model_path in df["Model Path"].unique():
            for eps in df["Epsilon"].unique():
                ...
                df_cumdf = pd.concat((df_cumdf, df_single), ignore_index=True)

    g = sns.FacetGrid(df_cumdf, col="Method")
    g = g.map_dataframe(
        sns.lineplot, x="Entropy", y="CDF", hue="Epsilon", ci=None
    ).add_legend()
...
def gen_ood_comparison(df_image, unc_key="Entropy"):
    ...
    df_mean_unc = (
        df_by_method[unc_key].mean().reset_index()
    )  # mean of all pixels per method
    df_mean_unc_img = (
        df_by_image[unc_key].mean().reset_index()
    )  # mean of all pixels in every method and image
    ...
    for method in df_mean_unc_img["Method"].unique():
        ...
        for ood in df_mean_unc_img["OOD"].unique():
            df_subset = df_mean_unc_img[
                (df_mean_unc_img["Method"] == method) & (df_mean_unc_img["OOD"] == ood)
            ]
            if len(df_subset) == 0:
                continue
            ...
            imgs_max[ood] = get_imgs_from_idx(
                idx=idxquantile(df_subset["Entropy"], 0.95)
            )
            imgs_min[ood] = get_imgs_from_idx(
                idx=idxquantile(df_subset["Entropy"], 0.05)
            )

        all_entropy_imgs = np.array(
            [[d[ood][2] for ood in d.keys()] for d in (imgs_max, imgs_min)]
        )
        ...
        for d in (imgs_max, imgs_min):
            for ood, (x, y, entropy) in d.items():
                id = os.path.join(
                    output_dir,
                    f"images/method_{method}_ood_{ood}_entropy_{entropy.mean()}",
                )
                cv2.imwrite(f"{id}_0.png", 255 * x)
                cv2.imwrite(f"{id}_1.png", apply_cmap(y, cmap=cv2.COLORMAP_JET))
                entropy = (entropy - entropy_bounds[0]) / (
                    entropy_bounds[1] - entropy_bounds[0]
                )
    ...
    unc_ = np.linspace(
        df_mean_unc_img[unc_key].min(), df_mean_unc_img[unc_key].max(), 200
    )
...
def predict(method, model, x, n_samples=10):
    ...
    if method == Model.Dropout:
        preds = tf.stack(
            [model(x, training=True) for _ in range(n_samples)], axis=0
        )  # forward pass
...
def create_adversarial_pattern(model, x, y):
    ...
    with tf.GradientTape() as tape:
        ...
        if isinstance(model, list):
            preds = tf.stack(
                [model_(x_, training=False) for model_ in model], axis=0
            )  # forward pass
